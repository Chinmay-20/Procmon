...
BOOL ProcessInfo::ProcessLog()
{
	...
	sprintf_s(FileName, "F:\\MarvelloousLog %02d_%02d_%02d_%s.txt", it.wHour, it.wMinute, it.wDay, month[it.wMonth - 1]);

	fp = fopen(FileName, "wb");
	if (fp == NULL)
	{
		cout << "Unable to create lag file" << endl;
		return FALSE;
	}
	else
	{
		cout << "Log file gets successfully created as :" << FileName << endl;
...
BOOL ProcessInfo::ReadLog(DWORD hr, DWORD min, DWORD date, DWORD month)
{
	...
	sprintf_s(FileName, "F:\\MarvelloousLog %02d_%02d_%02d_%s.txt", hr,min,date,montharr[month-1]);
	...
	while ((iret = fread(&fobj, 1, sizeof(fobj), fp)) != 0)
	{
		cout << "-----------------------------------------------------------------------";
...
BOOL ProcessInfo::KillProcess(char *name)
{
	...
	if (bret == FALSE)
	{
		cout << "ERROR:Unable to terminate the process";
		return FALSE;
	}
}
...
BOOL HardwareInfo()
{
	...
	cout << "Page Size:" << siSysInfo.dwProcessorType << endl;
...
void DisplayHelp()
{
	...
	cout << "kill :Closes the specified process" << endl;
	cout << "readlog :Display informaton from specified log file" << endl;
	...
	while (1)
	{
		...
		if (count == 1)
		{
			...
			else if (_stricmp(command[0], "readlog") == 0)
			{
				...
				if (bret == FALSE)
					cout << "ERROR: unable to read specified file" << endl;
				delete ppobj;
			}
